...
if genai is not None:
    try:
        ...
        logger.warning(f"{ERROR_ICON} Gemini 客户端初始化失败: {e}")
        client = None
else:
    logger.warning(f"{ERROR_ICON} Google Generative AI 包未安装，跳过 Gemini 客户端初始化")
...
def generate_content_with_retry(model, contents, config=None):
    """带重试机制的内容生成函数"""
    if client is None:
        logger.error(f"{ERROR_ICON} Gemini 客户端未初始化")
